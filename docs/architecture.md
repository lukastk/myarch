...
Waybar presents quiet status and critical Pocket controls. Richer operations go through `myarch menu` and fuzzel/application/TUI surfaces. `myarch` dispatches commands as argv through Python subprocess APIs; it does not build shell strings from selections.
