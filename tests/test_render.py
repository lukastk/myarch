...
class RenderTests(unittest.TestCase):
    ...
    def _run_app_flags(self, config_home: Path, *args: str) -> subprocess.CompletedProcess[str]:
        env = dict(os.environ, XDG_CONFIG_HOME=config_home.as_posix())
    ...
    def test_app_flags_follow_the_desktop_mode(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as temporary:
            ...
            self.assertEqual(["--ozone-platform-hint=auto"], self._launcher_flags(brave))
            self.assertEqual([], self._launcher_flags(obsidian))
            ...
            self.assertEqual(["--ozone-platform-hint=auto", blink], self._launcher_flags(brave))
            self.assertEqual([blink], self._launcher_flags(obsidian))
